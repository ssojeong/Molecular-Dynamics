...
class mb_base:
    ...
    def __init__(self,net1,net2,pwnet,ngrids,b):
    ...
    def evalall(self,net,q_list,p_list,l_list,tau):
        ...
        return y
    # ===================================================
    def prepare_input(self,q_list,p_list,l_list,ngrids,b): # make dqdp for n particles
        ...
        u_fields = self.gen_u_fields(q_list, l_list, u,ngrids)  # force fields

        # u_fields.shape is [nsamples, npartice, grids*DIM]

        v_fields = self.gen_v_fields(q_list,p_list,u, l_list,ngrids) # velocity fields - no change
        ...
        # x.shape = shape is [ nsamples, nparticle,  ngrids * DIM + ngrids * DIM ]

        #x = x.view(nsamples * nparticle, 2 * ngrids * DIM + 1)
        x = x.view(nsamples * nparticle, 2 * ngrids * DIM) # [batch, 24] -- good
        return x # 
    # ===================================================
    ...
    # ===================================================
    def ufields(self,q, l_list, u_list, ngrids):  # u_list = grid center position
        ...
        l_list = l_list[:, :, 0]
        l_list = l_list.view(nsamples, npar, 1)
        # l_list.shape = [nsample, nparticle, 1]
        dq = d*l_list
        # pwnet_input.shape = [batch, 2] - |dq|, |dp|

        dq.view(nsamples*nparticle*nparticle*ngrids,1] # |dq|
 
        dp =  -- we don't want pbc - particle momentum minus center particle momentum

        x = torch.cat((dq, dp), dim=-1)
         
        pair_pwnet = self.pwnet(x)

        #####pair13_ = -12 * (delta_grid_xi) / ( pow(l_list, 13) * pow(d,14) +  eps / l_list )
        #####pair07_  = -6 * (delta_grid_xi) / ( pow(l_list, 7) * pow(d,8) +  eps / l_list )
        ##### shape is [nsamples, nparticle, nparticle * ngrids, DIM]

        pair_pwnet = self.zero_ufields(pair_pwnet)

        #pair13 = self.zero_ufields(pair13_)
        #pair07 = self.zero_ufields(pair07_)

        #dphi_fields_list = 4 * (pair13 - pair07)

        dphi_fields = torch.sum(pair_pwnet, dim=1)  # np.sum axis=2 j != k ( nsamples-1)
        # dphi_fields.shape is [nsamples, nparticle * ngrids, DIM=2]
