...
class mb_base(fbase):
    ...
    def parameters(self): 
        return self.param
    # ===================================================
    #def sum_of_grid_force(self,q_input_list):
    #    nsamples,nparticles,_,_,dim = self.mask.shape

    #    q_input_stack = torch.stack(q_input_list,dim=0)
    #    q_input_mean  = torch.mean(q_input_stack,dim=0)
    #    x_mean = q_input_mean[:,0::2]
    #    y_mean = q_input_mean[:,1::2]
    #    x_mean_sum = torch.sum(x_mean,dim=1)
    #    y_mean_sum = torch.sum(y_mean,dim=1)
    ...
    def eval(self,q_input_list,p_input_list):
        ...
        #sum_force = self.sum_of_grid_force(q_input_list) # sum last q_list

        #test_sum_force = self.test_sum_of_grid_force(q_input_list) # sum last q_list
        #sumdiffsq = torch.sum((sum_force-test_sum_force)**2)
        #print('diff sq in mb_base.py ',sumdiffsq)
        #if sumdiffsq>1e-10: quit()
       
        return self.tau[netid]*( y )
        #return self.tau[netid]*( y + sum_force )
    # ===================================================
