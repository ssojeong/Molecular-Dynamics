...
if __name__=='__main__':
    ...
    traindict = {"loadfile"  : None,  # to load previously trained model
                 ...
                 "force_clip": 1,    # clamp the force within a range
                 "grad_clip" : 1,     # clamp the gradient for neural net parameters
                 "tau_long"  : 0.5,   # the large time step
                 "n_chain"   : 64,    # number of times to do integration before cal the loss
                 "ngrids"    : 6,     # for multibody interactions
                 "b"         : 0.2,   # grid lattice constant for multibody interactions
                 "lr"        : 1e-6,  # starting learning rate
                 "alpha_lr"  : 1e-8,  # lr for alpha for mbpw
                 "sch_step"  : 1,     # defunct: scheduler step
                 "sch_decay" : 1,     # defunct: scheduler decay
                 "reset_lr"  : False} 
    ...
    data = { "train_file": '../data_sets/n16anyrholt0.5everystps_nsamples600000.pt',
             "valid_file": '../data_sets/n16anyrholt0.5everystps_nsamples240000valid.pt',
             "test_file" : '../data_sets/n16anyrholt0.5everystps_nsamples240000valid.pt',
             "train_pts" : 12000,
             "vald_pts"  : 1200,
             "test_pts"  : 100,
             "batch_size": 28,
    ...
    maindict = { "start_epoch"  : 0,
                 "end_epoch"    : 10000,
                 "save_dir"     : 'ff_mbpw_lr1e-6nchain64npts0012000net3',
