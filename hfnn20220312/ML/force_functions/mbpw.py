...
class mbpw:
    ...
    def clamp_alpha(self):
        thrsh = 7.0
        if self.alpha1<-thrsh: self.alpha1.data = torch.tensor([-thrsh],requires_grad=True,device=mydevice.get())
        if self.alpha1> thrsh: self.alpha1.data = torch.tensor([ thrsh],requires_grad=True,device=mydevice.get()) 
        if self.alpha2<-thrsh: self.alpha2.data = torch.tensor([-thrsh],requires_grad=True,device=mydevice.get())
        if self.alpha2> thrsh: self.alpha2.data = torch.tensor([ thrsh],requires_grad=True,device=mydevice.get()) 
