...
if __name__ == '__main__':
    ...
    argv = sys.argv
    if len(argv) != 10:
        print('usage <programe> <npar> <rho> <nsteps> <gamma> <saved_model> <dpt> <region> <ylim> ' )
        quit()
    ...
    for i in temp_list:
        data = {
             #"energy1" : "../../../data_sets/gen_by_MD/noML-metric-st1e-4every0.1t100/n{}rho{}T{}/energytmax100.pt".format(npar,rho,T),
            "energy1": "../../../data_sets/gen_by_ML/lt0.1dpt100000_{}/n{}rho{}T{}/energy_gamma{}mb009_nsteps10000.pt".format(region, npar, rho, i, gamma),
             "energy2" : "../../../data_sets/gen_by_MD/noML-metric-lt0.01every1t0.7t1000/n{}rho{}T{}/energy_gamma{}_tmax1000.pt".format(npar,rho,i,gamma),
    ...
    print('tau long', tau_long, 'input seq idx', input_seq_idx )
    ...
    if (npar == 64 or npar == 128) and region == 'lg':
        ...
        for i in range(len(data0_list)):
            for j in range(0, nsamples, n_split):
                ...
                emean0["data0_" + str(i)] = de(e0["data0_" + str(i)][:,j:j+n_split], npar)
    ...
    for i in range(len(data1_list)):
        for j in range(0,nsamples,n_split):
            ...
            e_mean1["data1_" + str(i)] = de(e1["data1_" + str(i)][:,j:j+n_split], npar)
            e_mean2["data2_" + str(i)] = de(e2["data2_" + str(i)][:,j:j+n_split], npar)
    ...
    saved_txtfile = f'npar{npar}rho{rho}gamma{gamma}_e.txt'
    ...
    #         plt.errorbar([j + 1], e_mu0[j], yerr=e_se0[j], capsize=5,
    ...
    #     plt.errorbar([j+1],e_mu1[j], yerr=e_se1[j], capsize=5,elinewidth=0.5, color='k', markerfacecolor='none',marker='^',linestyle='none', markersize=14)
    #     plt.errorbar([j+1],e_mu2[j], yerr=e_se2[j], capsize=5,elinewidth=0.5, color='k', markerfacecolor='none',marker='x',linestyle='none', markersize=14)
    #
    ...
    # # plt.show()
    #
    # saved_dir = '/'.join(data['energy3'].split('/')[:-2]) + '/npar{}rho{}gamma{}nstep10000_e.pdf'.format(npar, rho, gamma)
