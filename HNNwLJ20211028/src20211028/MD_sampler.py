...
if __name__=='__main__':
    ...
    crash_checker                 = MD_parameters.crash_checker   # noML or train for ML : 'no', predict for ML 'yes'
    ...
    phase_space.set_l_list(boxsize)
    ...
    for i in range(n_out_files):
        ...
        # use to prepare label, histogram about crash data
            ...
            qpl_list, crash_flag, crash_iter, crash_ct = linear_integrator_obj.nsteps(hamiltonian_obj, phase_space, tau_cur,
                                                                                     save2file_strike, append_strike)
            # qpl_list is [nsamples, (q, p, boxsize), nparticle, DIM] append to trajectory length number of items
            ...
            if crash_history is False:
                ...
                qpl_list = torch.stack(qpl_list, dim=2)
                # qpl_list.shape = [nsamples, (q, p,boxsize), trajectory length, nparticle, DIM]
                ...
                if long_traj_save2file == 'yes':
                    ...
                    data_io.write_trajectory_qpl(tmp_filename, qpl_list, tau_short, tau_long)
            ...
            qpl_list = linear_integrator_obj.rm_crash_nsteps(hamiltonian_obj, phase_space, tau_cur, save2file_strike)
            print('qp list shape', qpl_list.shape)
            _filename = MD_output_basenames + '_metric.pt'
            data_io.write_trajectory_qpl(_filename, qpl_list, tau_short, tau_long)
