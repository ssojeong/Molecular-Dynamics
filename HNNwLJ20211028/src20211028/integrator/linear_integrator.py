...
class linear_integrator:
    ...
    def one_step(self, hamiltonian, phase_space, tau_cur):
        ...
        prev_l = phase_space.get_l_list()

        q_next, p_next, dhdq1, dhdq2, pred1, pred2  = self._integrator_method(hamiltonian, phase_space, tau_cur)
        # all shape is [nsamples, nparticle, DIM]

        crash_idx = self.crash_checker.check(phase_space, prev_q, prev_p, prev_l, dhdq1, dhdq2, pred1, pred2, hamiltonian)
        #print('q, p shape after crash checker ',phase_space.get_q().shape, phase_space.get_p().shape, 'crash_idx ', crash_idx)

        qpl_list = torch.stack((phase_space.get_q(), phase_space.get_p(), phase_space.get_l_list()), dim=1)
        # shape is [nsamples, (q,p,boxsize), nparticle, DIM]

        return qpl_list, crash_idx
    ...
    def nsteps(self, hamiltonian, phase_space, tau_cur, nitr, append_strike):
        ...
        qpl_list = [] # list to append qpl_list. qpl_list become empty if some samples have crash
        ...
        for t in range(nitr):
            ...
            nxt_qpl, crash_idx = self.one_step( hamiltonian, phase_space, tau_cur)
        ...
        return qpl_list, crash_flag, crash_iter, crash_ct
    ...
    def rm_crash_nsteps(self, hamiltonian, phase_space, tau_cur, nitr):
        ...
        init_qpl = torch.stack((phase_space.get_q(), phase_space.get_p(), phase_space.get_l_list()), dim=1)
        ...
        for t in range(nitr):
            ...
            nxt_qpl, crsh_set = self.one_step(hamiltonian, phase_space, tau_cur)
            # nxt_qp.shape is [nsamples, (q, p, boxsize), nparticle, DIM]
            ...
            if t == 0:
                if crsh_set is not None:
                    ...
                    qpl_list = torch.stack((init_qpl[diff_list], nxt_qpl),dim=2)
                    # qp_list.shape is [nsamples, (q, p, boxsize), trajectory length, nparticle, DIM]

                else:
                    # take index from inital state and then stack with nxt state
                    qpl_list = torch.stack((init_qpl, nxt_qpl), dim=2)

            else:
                if crsh_set is not None:
                    nxt_qpl = torch.unsqueeze(nxt_qpl, dim=2)
                    # nxt_qp.shape is [nsamples, (q, p), 1, nparticle, DIM]

                    # take no crash index from qp_list and then stack with nxt_qp
                    qpl_list = torch.cat((qpl_list[diff_list],nxt_qpl),dim=2)
                    ...
                    qpl_list = torch.cat((qpl_list, nxt_qpl), dim=2)
