...
class metropolis_mc:
    ...
    def mcmove(self, hamiltonian, phase_space) :
        ...
        curr_q[:, trial] = old_q + (torch.rand(1, MC_parameters.DIM) - 0.5) * boxsize * MC_parameters.dq

        boxsize = boxsize.expand(curr_q.shape)
        # boxsize.shape is [1, npaticle, DIM]

        phase_space.adjust_real_q(curr_q, boxsize)
    ...
    def step(self, hamiltonian, phase_space):
       ...
       '''
        ...
        for z in range(0, MC_parameters.nsamples):
            ...
            phase_space.set_l_list(torch.tensor(self.boxsize))
            ...
            print('finished taking {} configuration, '.format(z), 'temp: ', MC_parameters.temperature, 'Accratio: ', ACCRatio[z], 'spec: ', spec[z], 'dq: ', MC_parameters.dq, 'rho: ', MC_parameters.rho,  'Discard: ', MC_parameters.DISCARD, 'time: ', end-start)
