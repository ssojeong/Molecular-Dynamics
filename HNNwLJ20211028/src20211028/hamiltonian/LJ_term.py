...
class LJ_term:
    ...
    def __init__(self, epsilon, sigma):
        ...
        assert (LJ_term._obj_count <= 3),type(self).__name__ + " has more than one object"
    ...
    def derivate_phi_fields(self, xi_space, grids_list):
        ...
        nsamples, npar, DIM = xi_state.shape

        boxsize = xi_space.get_l_list()
        # boxsize.shape is [nsamples, nparticle, DIM]

        l_reduced = torch.unsqueeze(boxsize, dim=2)
        l_reduced = l_reduced.repeat_interleave(grids_list.shape[1],dim=2)
        # l_reduced.shape is [nsamples, nparticle, nparticle * ngrids, DIM]

        l_reduced = torch.zeros(l_reduced.shape)
        l_reduced.fill_(1)
        # l_reduced.shape is [nsamples, nparticle, nparticle * ngrids, DIM]

        delta_grid_xi, d = self.dpair_pbc.paired_grid_q(xi_space, grids_list, l_reduced)
        ...
        boxsize = boxsize.view(nsamples, npar, 1 , 1)
        # boxsize.shape is [nsamples, nparticle, 1 ,1 ]
    ...
    def energy(self, xi_space):
        ...
        # a12.shape and a6.shape are [nsamples, nparticle,1]
        ...
        term_dim = torch.sum(a12 * s12, dim=-1) -  torch.sum(a6 *s6, dim=-1)
    ...
    def evaluate_derivative_q(self,xi_space):
        ...
        boxsize = torch.mean(boxsize, dim=-1)
        # boxsize.shape is [nsamples, nparticle]
        boxsize = boxsize.view(nsamples, nparticle,1 ,1)
        # boxsize.shape is [nsamples, nparticle, 1 ,1 ]

        a12 = (4 * self._epsilon * pow(self._sigma, 12)) / pow(boxsize, 13)
        a6 = (4 * self._epsilon * pow(self._sigma, 6)) / pow(boxsize, 7)
        # shape is [nsamples, nparticle, 1 ,1 ]
