...
class lennard_jones:
    ...
    def __init__(self,sigma = 1,epsilon = 1):
        ...
        assert (lennard_jones._obj_count <= 3),type(self).__name__ + " has more than one object"
    ...
    def dimensionless(self, phase_space):
        ...
        self.dimensionless_phase_space.set_l_list(boxsize)
    ...
    def dimensionless_grids(self, grid_list, boxsize): 
        ...
        boxsize = boxsize.repeat_interleave(grid_list.shape[1]//boxsize.shape[1],dim=1)
        boxsize = torch.unsqueeze(boxsize,dim=-1)
        # boxsize.shape is [nsamples, nparticle * grids18, 1]
