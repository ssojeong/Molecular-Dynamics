...
for mcs in range(MCS):
    ...
    for _ in range(N):
      ...
      #pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)    #N=2 T = 0.5,0.55,0.6
      #pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)*0.6  #N=2 T = 0.35,0.4,0.45
      #pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)*0.3  #N=2 T = .25,.3
      #pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)*0.2 #N=2 T = .15,2
      #pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)*0.1  #N=2 T = .05,1
      pos[trial]= pos_old +  (np.random.rand(1,DIM)-0.5)*0.03  #N=2 T = .0.01
...
np.save('./filesave/' + 'N{}_T{:.2f}_pos_sampled.npy'.format(N,T),np.array(pos_xy))
...
#plt.xlim(0,BoxSize)
#plt.ylim(0,BoxSize)
#plt.title(r'$\rho$={}, BoxSize={:.2f}, T={}'.format(rho,BoxSize,T),fontsize=15)
#for i in range(N):
...
# Uij for mc step
plt.title(r'$\rho$={}, BoxSize={:.2f}, T={}, AccRatio={:.3f}'.format(rho,BoxSize,T,ACCRatio),fontsize=15)
plt.plot(U,'k-')
plt.xlabel('mcs',fontsize=20)
plt.ylabel(r'$U_{ij}$',fontsize=20)
plt.savefig('./filesave/' + r'N{}_rho{}_T{:.2f}.png'.format(N,rho,T))
plt.close()
