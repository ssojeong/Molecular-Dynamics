...
rho     = 0.550  # rho_c=0.366(9) for 2d LJ liquid 
...
for  T in np.linspace(0.01, 0.74, 20):
#for  T in np.linspace(0.1, 1.6, 50):
 ...
 for mcs in range(MCS):
  for _ in range(N):
    ...
    #pos[trial]=np.random.rand(1,DIM)*0.1
 ...
 spec = (TE2sum/Nsum - TE1sum*TE1sum/Nsum/Nsum)/T/T/N
 ACCRatio = ACCsum / ACCNsum 
 print ("%20.10f  %20.10f  %20.10f"  %(T, spec, ACCRatio) )
