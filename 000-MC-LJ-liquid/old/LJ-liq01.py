    ...
    for i in range(N-1):
        for j in range(i+1,N): #i+1 to N ensures we do not double count
            ...
            if(Rsqij < Rcutoff**2):
                ...
                phi = epsilon*(4.0*(rm12-rm6)) # 4[1/r^12 - 1/r^6] - phi(Rc) 
...
for k in range(Nsteps):
  ...
  acc, pot_old = Compute_Forces(pos,acc,ene_pot,epsilon,BoxSize,DIM,N) 
  print("old : %10i %20.10f %20.10f" %(k, acc[0,0]  ,pot_old))

  # update pos 
  pos[1,0:1]=uniform(-0.3,0.3)

  # Compute forces a(t+dt),ene_pot,virial
  acc, pot_new = Compute_Forces(pos,acc,ene_pot,epsilon,BoxSize,DIM,N) 

  print("new: %10i %20.10f %20.10f" %(k, acc[0,0]  ,pot_new))
