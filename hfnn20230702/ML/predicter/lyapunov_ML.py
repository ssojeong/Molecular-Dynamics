...
if __name__ == '__main__':
    ...
    qpl_traj = qpl_list['qpl_trajectory']
    qpl_inputs = qpl_traj[:,:,0:traj_len_idx,:,:]
    #shape [nsamples, 3, traj, nparticles, dim]
    print('load data init file : ',  qpl_inputs.shape)
    qpl_labels = qpl_traj[:, :,label_idx, :, :]
    
    dataset = TensorDataset(qpl_inputs, qpl_labels)
    ...
    count = 0
    for qpl_input, qpl_label in test_loader:
      
        print('sample for dt traj ====' , qpl_input.shape )
        ...
        for k in range(t_thrsh): # thrsh not over t incremented until eps
            ...
            if avg_dq_list < eps: # 4e-2 4e-3
                ...
                print('steps', k+1, R_sample_append)
                data = {'t_accum': k+1, 'R_sample_append': R_sample_append}
                torch.save(data, saved_filename+ '_id{}.pt'.format(count))
                print('save file dir', saved_filename + '_id{}.pt'.format(count))
                count += 1
                #quit()
                break
