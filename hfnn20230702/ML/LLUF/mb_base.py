...
class mb_base(fbase):
    ...
    def __init__(self,mbnet_list,pwnet_list,ngrids,b,nnet,tau_init):
        #super().__init__(mbnet_list,nnet,tau_init) # 20230701 hk
    ...
    def max_abs_grad(self,name): # SJ update
        ...
        mb_name = name + '-mb4mb list len' + str(len(self.mbnet_list))
        mb_max_grad = self.get_max_abs_grad(mb_name,self.mbnet_list)

        return max(pw4mb_max_grad, mb_max_grad)
    # ===================================================
    def eval_mode(self):
    ...
    def eval_base(self,net_id,q_input_list,p_input_list, q_pre): # SJ coord
        ...
        target_net = self.mbnet_list[net_id]
        return self.evalall(target_net,x,dq, q_pre)
    ...
    #     #print('mb_base.py use pwnet_id ',pwnet_id)
    #     return self.prepare_q_input_net(q_list,p_list,l_list,self.pwnet_list[pwnet_id])
    # ===================================================

    # q_list.shape = [nsamples,nparticles,DIM] - one time point
    # p_list.shape = [nsamples,nparticles,DIM] - one time point
    def prepare_q_input(self,q_list,p_list,l_list,pwnet): # make dqdp for n particles
        # u_fields.shape = # u_fields.shape  [nsamples, npartice, ngrids*DIM], ngrids=6, DIM=2
        ...
        # u_fields.shape  [nsamples, npartice, ngrids*DIM]
        return u_fields.view(self.nsamples * self.nparticles, self.ngrids * self.DIM)  # [batch, 12] -- good
    # ===================================================
    def prepare_p_input(self,q_list,p_list,l_list): # make dqdp for n particles
        ...
        return v_fields.view(self.nsamples * self.nparticles, self.ngrids * self.DIM) # [batch, 12] -- good
    # ===================================================
    ...
    def ufields(self,q, p, l_list, u_list, pwnet, ngrids):  # u_list = grid center position
        ...
        del_p = _delta_state(p) #  shape is [nsamples, nparticles, nparticles, DIM]
    ...
    def gen_v_fields(self,q,p,u_list,l_list,ngrids): # velocity fields
        ...
        return relative_p
    # ============================================
    def test_sum_of_grid_force(self,q_input_list):
        nsamples,nparticles,_,_,dim = self.mask.shape
        
        q_sum = q_input_list[0].clone()
        for q_input in q_input_list[1:]:
            q_sum = q_sum + q_input
        
        q_mean = q_sum / len(q_input_list)
        
        x_mean_sum = q_mean[:,0]+q_mean[:,2]+q_mean[:,4]+q_mean[:,6]+q_mean[:,8]+q_mean[:,10]
        y_mean_sum = q_mean[:,1]+q_mean[:,3]+q_mean[:,5]+q_mean[:,7]+q_mean[:,9]+q_mean[:,11]
            
        ret = torch.stack((x_mean_sum,y_mean_sum),dim=1)
        ret = ret.view([nsamples,nparticles,dim])
        return ret 
