...
if __name__ == '__main__':
    ...
    if len(argv) != 4:
        print('usage <programe> <filename at lt> <filename at st> <temp>')
        quit()
    ...
    traj_lt = data1['qp_trajectory']
    traj_st = data2['qp_trajectory']
    # shape = [nsamples, (q,p), trajectory, nparticles, DIM]

    nsamples, _, iterations, nparticle, _ = traj_lt.shape
    ...
    # plt.ylim(-0.01,1e-10)
    plt.plot(t,avg_del_q_particle_sample[:max_ts_cut].detach().numpy(), label = 'ML(0)-hard1')
    plt.grid()
    plt.legend()
    ...
    plt.plot(t,avg_del_p_particle_sample[:max_ts_cut].detach().numpy(), label = 'ML(0)-hard1')
    plt.grid()
    plt.legend()
    # plt.xscale('log',base=2)
    # plt.yscale('log',base=2)
    ...
    plt.plot(t,avg_del_qp_particle_sample[:max_ts_cut].detach().numpy(), label = 'ML(0)-hard1-nsamples{}'.format(nsamples))
    # plt.xscale('log',base=2)
    # plt.yscale('log',base=2)
