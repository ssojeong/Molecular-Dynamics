...
class derivate_phi_fields:
    ...
    def __init__(self, noML_hamiltonian, dgrid, ngrids, dphi_maxcut=108.35):  # rij=0.9 -> dphi ~ 108.35
        ...
        derivate_phi_fields._obj_count += 1
        assert(derivate_phi_fields._obj_count == 1), type(self).__name__ + ' has more than one object'
    ...
    def v_ngrids(self, phase_space):
        ...
        p_list_ = phase_space.get_p()
        boxsize =phase_space.get_boxsize()

        nsamples, nparticle, DIM = q_list.shape

        _, d = self.dpair_pbc.paired_grid_q(q_list, self.grids_list, boxsize)
        ...
        p_list = torch.unsqueeze(p_list_, dim=2)
        ...
        # p_grids.shape [nsamples, npartice, grids, DIM]
        # p_list.shape is [nsamples, nparticle, 1, DIM]
    ...
    def weight_thrsh(self, d, boxsize):
        ...
        # w.shape is [nsamples, nparticle, nparticle * ngrids, 1]

        max_d = torch.sqrt(2 * torch.tensor(0.5 * boxsize) ** 2)
