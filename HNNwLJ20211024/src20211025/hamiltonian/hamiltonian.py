...
class hamiltonian:
    ...
    def __init__(self):
        ...
        assert (hamiltonian._obj_count <= 3),type(self).__name__ + " has more than one object"
    ...
    def dHdq1(self, phase_space):
        '''
        dHdq1 function to get the dHdq1, pred_dHdq1
        set pred_dHdq1 zero when run only noML

        Returns
        -------
        dHdq1, pred dHdq1 : torch.tensor
        shape is [nsamples, nparticle, DIM]
        '''
        zero_shape = torch.zeros(phase_space.get_q().shape)
        phase_space.set_ml_dhdq1(zero_shape)
        return self.dHdq(phase_space), zero_shape

    def dHdq2(self, phase_space):
        '''
        dHdq2 function to get the dHdq2, pred_dHdq2
        set pred dHdq2 zero when run only noML

        Returns
        -------
        dHdq2, pred_dHdq2 : torch.tensor
        shape is [nsamples, nparticle, DIM]
        '''

        zero_shape = torch.zeros(phase_space.get_q().shape)
        phase_space.set_ml_dhdq2(zero_shape)
        return self.dHdq(phase_space), zero_shape
