...
class check4particle_soft_crash:
    ...
    def check(self, phase_space, prev_q, prev_p, prev_dHdq1, prev_dHdq2, prev_pred1, prev_pred2, hamiltonian):
        ...
        # d = d * boxsize
        #
        # check_r =  (d < self.rthrsh0) & ( self.rthrsh2 < d)
        #
        # if check_r.any() == True:
        ...
        #     crash_flag = True
        #
        # garbage_r = d >= self.rthrsh2
        #
        # if garbage_r.any() == True:
        #     s_idx = torch.where(garbage_r)
        #     s_idx = torch.unique(s_idx[0], sorted=True)
        #     print('check_r too high, bb to garbage: ', d[s_idx], 'sample idx is ', s_idx)
        #     gar_idx.append(s_idx)
        #     garbage_flag = True

        # check energy per par more than ethrsh and less than ethrsh * 1.5
        check_e = (next_energy_per_particle > self.pothrsh) & (self.pothrsh2 > next_energy_per_particle)
        ...
        # check p next is more than pthrsh0 and less than pthrsh2
