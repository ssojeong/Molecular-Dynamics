...
def count2rdf(count, bin_edges, rho, n_sample, num_mol):
    grbin = []
    bin_width = bin_edges[1] - bin_edges[0]
    # print(type(count_array), type(bin_edges))
    for c, r in zip(count, bin_edges[:-1]):
...
def q2dis(q, num_mol, n_bins, r_min, r_max, box_size):
    dis_list = []
...
def pair_distribution_function(file_list, op_name):
    ...
    for idx, f in enumerate(file_list):
        ...
        counts, bin_edges = q2dis(q, num_mol, n_bins, r_min, r_max, box_size)
    ...
    grbin = count2rdf(count_array, bin_edges, rho, n_sample, num_mol)
    print(type(count_array), len(count_array), len(bin_edges), len(grbin))
...
if __name__ == '__main__':
    ...
    pair_distribution_function(file_list, f'../../../../Data/LLUF/300k_LLUF_ws8_{pt}_histo.pt')
