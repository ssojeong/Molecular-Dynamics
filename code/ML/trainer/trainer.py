...
from ML.networks.PWNet3DH2O import PWNet3DH2O
...
class trainer:
    ...
    def __init__(self,train_dict,loss_dict):
        ...
        self.LLUF_update_p_obj = HalfStepUpdate(self.prepare_data_obj, single_particle_net_list[0],
                                                multi_particle_gnn_net_list[0],
                                                readout_step_mlp_net_list[0],
                                                train_dict["tau_init"])
        ...
        if train_dict["optimizer"] == 'SGD':
            ...
            raise ValueError(f"Invalid optimizer given for optimizer: {train_dict['optimizer']}")

        self.weights = train_dict['loss_weights']
        print(f"loss weight .... {self.weights}...")
...
# ==========================================================
    ...
    def net_builder(self, train_dict):
        ...
        # LW: call PWNet3DH2O instead of PWNet
        # TODO input_dim
        prepare_data_net = mydevice.load(PWNet3DH2O(8, pw4mb_dim, train_dict['net_nnodes']))

        assert train_dict['single_particle_net_type'] == 'transformer_type',\
            f"single_particle_net_type should be transformer_type, got {train_dict['single_particle_net_type']}"
        print("single particle net type transformer ..........")

        # two mb transformer net
        single_par_transformer_kwargs = {
            'input_dim': input_dim,
            'output_dim': train_dict["d_model"],
            ...
            'p': train_dict["net_dropout"]}
        single_particle_net_list.append(mydevice.load(SingleParticleTransformerNet(**single_par_transformer_kwargs)))
        single_particle_net_list.append(mydevice.load(SingleParticleTransformerNet(**single_par_transformer_kwargs)))

        assert train_dict["multi_particle_net_type"] == 'gnn_identity', f"multi_particle_net_type should be gnn_identity"
        ...
        readout_step_mlp_kwargs = {'input_dim': train_dict["d_model"], 'output_dim': output_dim,
                                   'nnodes': train_dict["net_nnodes"][0], 'p': train_dict["net_dropout"]}
        if train_dict["readout_step_net_type"] == 'mlp_identity':
            print("readout step net type mlp identity..........")
            readout_step_mlp_kwargs['readout'] = False
