...
def plot_pairs(dd,npar,rho, boxsize):
    ...
    fig.suptitle(r"boxsize={:.3f} npar={} $\rho$={}".format(boxsize.item(), npar, rho), fontsize=16)
    for i, ax in enumerate(axs.flatten()):
        ax.set_title('dq min {:.2f}; max {:.2f}'.format(torch.min(dd[i])*boxsize.item(),torch.max(dd[i])*boxsize.item()))

        ax.hist(boxsize.item()*dd[i].reshape(-1).numpy(),bins=100,density=True, edgecolor='black', label=f'T={temp_list[i]}')
        kde = gaussian_kde(boxsize.item()*dd[i].reshape(-1).numpy())
        x_vals =  np.linspace(torch.min(boxsize.item()*dd[i].reshape(-1)), torch.max(boxsize.item()*dd[i].reshape(-1)), 200)
        ax.plot(x_vals, kde(x_vals), color='darkblue', linewidth=2) #label='KDE (smoothed curve)'

        ax.set_xlabel(r'$dq$',fontsize=16)
        ax.legend()
