...
if __name__ == '__main__' :
    ...
    #print(qrmse_t)
    for i in range(1):
        ax[i, 0].plot(tepoch, qrmse_t[:,0], 'bo-', label='train', zorder=2)
        ax[i, 0].plot(vepoch, qrmse_v[:,0], 'o-', label='valid', c='orange', zorder=1)
        ax[i, 1].plot(tepoch, prmse_t[:,0], 'bo-', label='train', zorder=2)
        ax[i, 1].plot(vepoch, prmse_v[:,0], 'o-', label='valid', c='orange', zorder=1)
    ...
    for i in range(4):
        ax[1, i].set_xlabel('epochs', fontsize=15)
