...
if __name__ == '__main__':
    ...
    print(load_files, name, loss_weights, title)
    ...
    #print(qrmse_t)
    # for i in range(1):
    ax[0].plot(tepoch, qrmse_t[:,0], 'bo-', label='train', zorder=2)
    ax[0].plot(vepoch, qrmse_v[:,0], 'o-', label='valid', c='orange', zorder=1)
    ax[1].plot(tepoch, prmse_t[:,0], 'bo-', label='train', zorder=2)
    ax[1].plot(vepoch, prmse_v[:,0], 'o-', label='valid', c='orange', zorder=1)
    ...
    ax[0].set_ylabel(r'$L_1$', fontsize=15)

    ax[0].set_title('q L2 norm', fontsize=15)
    ax[1].set_title('p L2 norm', fontsize=15)
    ...
    for j in range(2):
        ax[j].set_xlabel('epochs', fontsize=15)
