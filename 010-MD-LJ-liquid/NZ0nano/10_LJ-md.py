    ...
    for i in range(N-1):
        for j in range(i+1,N): #i+1 to N ensures we do not double count
            ...
            if(Rsqij < Rcutoff**2):
                ...
                rm6  = rm2**3.0 # 1/r^6
                rm12 = rm6**2.0 # 1/r^12
                LJij = epsilon*(4.0*(rm12-rm6)-phicutoff) # 4[1/r^12 - 1/r^6]
...
BoxSize    = 10.0
volume     = BoxSize**DIM
rho        = N/volume  # rho_c=0.366(9) for 2d LJ liquid 
...
vel     = np.random.normal(0, np.sqrt(TRequested), (N,DIM) )
#vel     = vel/BoxSize
