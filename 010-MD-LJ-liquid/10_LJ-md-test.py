    ...
    for k in range(pos.shape[0]):
        for i in range(N-1):
            for j in range(i+1,N): #i+1 to N ensures we do not double count
                Sij = pos[k,i]-pos[k,j] # Distance in box scaled units
                print(pos[k,i])
                print(pos[k,j])
                print(Sij)
...
def randNormal(pos): 
    ...
    random_1 = random_1.reshape(-1,pos[0])
    random_2 = random_2.reshape(-1,pos[0])
########## Main ###############################
############################################################################
...
Nsteps     =   100
deltat     = 0.01
TRequested =  6.5   # Reduced temperature
DumpFreq   =   10   # Save the position to file every DumpFreq steps
R_adj      = 0.0
#T          = 0.522  # Tc = 0.522(2) for 2d LJ lquid
rho        = 0.2  # rho_c=0.366(9) for 2d LJ liquid 
...
for k in range(Nsteps):
    if(k%(Nsteps/100)==0) :
        print("%10i steps of all %10i steps : %10.3f %%  proceeding... " \
                %(k, Nsteps, k/Nsteps*100.))
